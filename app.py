...
def pdf_report(avg_glucose, latest_glucose, tir, hba1c, selected_dt, current_glucose,
    ...
    elems.append(Paragraph("🍽 Diet Suggestions", styles["Heading2"]))
...
# Tabs
# =========================
tab1, tab2, tab3, tab4 = st.tabs(["📊 Analytics", "🍽 Diet", "💉 Insulin", "📄 Report"])
...
with tab1:
    ...
    daily_avg = ddf.groupby("_Date")[g_col].mean().rename("Average").reset_index().rename(columns={"_Date": "Date"})
...
with tab3:
    ...
    with colB:
        isf = st.number_input("⚖ ISF (mg/dL per unit)", value=14.13, min_value=5.0, max_value=80.0, step=0.01, format="%.2f")
